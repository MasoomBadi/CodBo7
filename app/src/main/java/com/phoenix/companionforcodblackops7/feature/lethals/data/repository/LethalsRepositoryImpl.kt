package com.phoenix.companionforcodblackops7.feature.lethals.data.repository
...
class LethalsRepositoryImpl @Inject constructor(
        ...
        return realm.query<DynamicEntity>("tableName == $0", "lethals_mp")
            ...
            .map { results ->
                results.list.mapNotNull { entity ->
                    try {
                        deserializeLethal(entity)
                    } catch (e: Exception) {
                        Timber.e(e, "Failed to deserialize lethal: ${entity.id}")
                        null
                    }
    ...
    override fun getLethalById(id: Int): Flow<Lethal?> {
        return getAllLethals().map { lethals ->
            lethals.find { it.id == id }
        }
    }

    private fun deserializeLethal(entity: DynamicEntity): Lethal {
